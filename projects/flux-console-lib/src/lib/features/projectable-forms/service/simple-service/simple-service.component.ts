...
export class SimpleServiceComponent extends ProjectableForm {
  ...
  async saveConfig(
    ...
    if (configResult.error) {
      ...
      this[cliCommandProp] = `flux edge create config '${configApiData.name}' '${configApiData.configTypeId}' '${JSON.stringify(configApiData.data)}'`;
  ...
  async saveService(summaryData) {
    ...
    if (serviceResult.error) {
      ...
      this.serviceCliCommand = `flux edge create service '${this.serviceApiData.name}' ${cliConfigIds}`
  ...
  async savePolicy(summaryData, type = 'Dial') {
    ...
    const cliCommand = `flux edge create service-policy '${policyApiData.name}' ${type} --semantic AnyOf --service-roles '@${this.serviceId}' --identity-roles '${cliRoles}'`;
    if (type === 'Dial') {
