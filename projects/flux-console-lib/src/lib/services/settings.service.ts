...
export class SettingsService extends SettingsServiceClass {
    ...
    override controllerSave(name: string, url: string) {
        ...
        firstValueFrom(this.httpClient.get(callUrl).pipe(catchError((err: any) => {
            throw "Flux Controller not Online: " + err?.message;
        }))).then((body: any) => {
            try {
                if (body.error) {
                  let growlerData = new GrowlerModel(
                    'error',
                    'Error',
                    'Invalid Flux Controller: ' + body.error,
                  );
                  this.growlerService.show(growlerData);
                    ...
                    if (body?.data?.apiVersions?.edge?.v1 != null) {
                      ...
                      let growlerData = new GrowlerModel(
                        'error',
                        'Error',
                        'Invalid Flux Controller: ' + JSON.stringify(body),
                      );
                      this.growlerService.show(growlerData);
                    }
                }
            ...
            .pipe(
                ...
                catchError((err: any) => {
                    ...
                    const growlerData = new GrowlerModel(
                        ...
                        'Error',
                        `Flux Controller`,
                        `Failed to connect to Flux Controller: ${err?.message}`,
                    );
